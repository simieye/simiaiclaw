...
const ECOMMERCE_PLATFORMS: Platform[] = [
  ...
  },
  {
    ...
    docsSections: ['US蟹Proxy API', 'Content API', 'Orders API'],
  },
];
          ...
          <div className="flex gap-2 mt-4">
            {platform.loginUrl && (
              <a
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-cyan-600/80 hover:bg-cyan-500 text-white rounded-lg py-2.5 text-sm font-medium transition-colors"
              >
                🔑 登录 / 授权
              </a>
            )}
            {platform.developerUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 flex items-center justify-center gap-2 bg-emerald-600/80 hover:bg-emerald-500 text-white rounded-lg py-2.5 text-sm font-medium transition-colors"
              >
                🛠️ 开发者控制台
              </a>
            )}
      ...
      <div className="flex gap-2 mt-3 pt-3 border-t border-slate-700/30">
        {platform.loginUrl && (
          <a
            ...
            className="flex-1 flex items-center justify-center gap-1 bg-slate-800/70 hover:bg-cyan-600/20 border border-slate-700/50 hover:border-cyan-500/40 rounded-lg py-1.5 text-xs text-slate-300 hover:text-cyan-300 transition-colors"
          >
            🔑 <span>登录/授权</span>
          </a>
        )}
        {platform.developerUrl && (
          <a
            ...
            rel="noopener noreferrer"
            onClick={e => e.stopPropagation()}
            className="flex-1 flex items-center justify-center gap-1 bg-slate-800/70 hover:bg-emerald-600/20 border border-slate-700/50 hover:border-emerald-500/40 rounded-lg py-1.5 text-xs text-slate-300 hover:text-emerald-300 transition-colors"
          >
            🛠️ <span>开发者控制台</span>
          </a>
        )}
